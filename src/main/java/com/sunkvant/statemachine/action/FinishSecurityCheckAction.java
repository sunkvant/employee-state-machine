...
@Slf4j
public class FinishSecurityCheckAction implements Action<EmployeeState, EmployeeEvent> {
    @Override
    public void execute(StateContext<EmployeeState, EmployeeEvent> stateContext) {
        ...
        log.info(String.format("Employee security check finished: [id=%s]", employeeId));
    }
}
